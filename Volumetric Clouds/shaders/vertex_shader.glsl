...
void main()
{
	...
	vec4 ray_start_vector = inverse_projection_matrix * vec4(input_vertex, -1.0, 1.0);
	ray_start_vector /= ray_start_vector.w;

	vec4 ray_end_vector = inverse_projection_matrix * vec4(input_vertex, 1.0, 1.0);
	...
	gl_Position = vec4(input_vertex, -1.0, 1.0);
}